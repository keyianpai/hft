...
bool Strategy::MidSell() {
  ...
  if (mid < down_diff - abs(current_pos)*increment) {
    if (mode == "real") {
...
bool Strategy::MidBuy() {
  double mid = (shot_map[main_ticker].bids[0]+shot_map[main_ticker].asks[0])/2
             - (shot_map[hedge_ticker].bids[0]+shot_map[hedge_ticker].asks[0])/2;
  if (mid > up_diff + abs(current_pos)*increment) {
    if (mode == "real") {
...
void Strategy::Flatting() {
  ...
  if (HitMean()) {
    if (current_pos > 0) {  // close buy
      ...
      Order* o = NewOrder(main_ticker, OrderSide::Sell, abs(current_pos), false, false);  // close
      ...
      shot_map[main_ticker].Show(stdout);
      shot_map[hedge_ticker].Show(stdout);
      Order* o = NewOrder(main_ticker, OrderSide::Buy, abs(current_pos), false, false);  // close
      closed_size += abs(current_pos);
      if (mode == "test") {
        ExchangeInfo info;
        info.trade_size = o->size;
...
void Strategy::Run() {
  ...
  if (!MidBuy() && abs(current_pos) < max_pos) {  // open sell
    ...
    shot_map[main_ticker].Show(stdout);
    shot_map[hedge_ticker].Show(stdout);
    Order* o = NewOrder(main_ticker, OrderSide::Sell, 1, false, false);
    if (mode == "test") {
      ExchangeInfo info;
      info.trade_size = o->size;
      DoOperationAfterFilled(o, info);
  ...
  if (!MidSell() && abs(current_pos) < max_pos) {  // open buy
    ...
    shot_map[main_ticker].Show(stdout);
    shot_map[hedge_ticker].Show(stdout);
    Order* o = NewOrder(main_ticker, OrderSide::Buy, 1, false, false);
    if (mode == "test") {
      ExchangeInfo info;
      info.trade_size = o->size;
      DoOperationAfterFilled(o, info);
